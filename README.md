...
- 💾 Client-side tile cache (IndexedDB), shared across all cards in the browser – cuts down repeat tile downloads, works over plain `http://` too; size limit and a clear-cache button with hit/miss stats live in the card editor
...
```yaml
resources:
  - url: /local/mapy-map-card.js?v=0.4.1
    type: module
```
...
- The tile cache lives in the browser's IndexedDB (per browser/device, like the switcher picks above) – chosen over the Cache Storage API specifically because Cache Storage requires HTTPS/localhost, and most home HA installs are reached over plain `http://` on the LAN. Falls back to plain network requests automatically if IndexedDB is unavailable or blocked (private browsing, older embedded browsers) – caching can never break the map.
